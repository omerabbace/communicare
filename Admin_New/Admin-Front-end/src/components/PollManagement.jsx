...
import { Pie } from "react-chartjs-2";
import Sidebar from "./Sidebar";
import "../styles/PollManagement.css"; // Updated CSS file
import { BASE_URL } from "../config";
import { Chart as ChartJS, ArcElement, Tooltip, Legend } from "chart.js";
...
const PollManagement = () => {
  ...
  const [error, setError] = useState("");
  const [showActivePolls, setShowActivePolls] = useState(true);
  const [openPollDetails, setOpenPollDetails] = useState({});
  ...
  const getChartData = (pollId) => {
    ...
    return {
      labels: results.map((r) => `${r.optionText} (${r.count} votes)`),
      datasets: [
        {
          label: "Votes",
          data: results.map((r) => r.count || 0),
          backgroundColor: ["#ff6384", "#36a2eb", "#ffcd56", "#4bc0c0", "#9966ff", "#ff9f40"],
          hoverBackgroundColor: ["#ff8396", "#58c3ff", "#ffe083", "#66e6d8", "#b399ff", "#ffaf69"],
          borderWidth: 1,
          borderColor: "#fff",
        },
      ],
  ...
  return (
    <div className="poll-management-container">
      <Sidebar />
      <div className="content">
        <header className="header">
          <h2>Manage Polls</h2>
          <button className="toggle-button" onClick={togglePollView}>
            {showActivePolls ? "Show Ended Polls" : "Show Active Polls"}
          </button>
        </header>
        <div className="poll-container">
          {showActivePolls ? (
            <>
              ...
              {activePolls.map((poll, index) => (
                <div key={poll._id} className="poll-card">
                  <p className="poll-title" onClick={() => togglePollDetails(poll._id)}>
                    {index + 1}. {poll.name}
                  </p>
                  {openPollDetails[poll._id] && (
                    <div className="poll-details">
                      ...
                      <p>Status: {poll.disableVotes ? "Disabled" : "Enabled"}</p>
                      <button
