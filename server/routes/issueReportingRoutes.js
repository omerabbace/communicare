    ...
    router.get('/issues/requiring-volunteers', isLogin, issueController.getIssuesRequiringVolunteers);
    ...
    router.get('/leader-issues', isLogin, isVolunteer, issueController.getLeaderIssues); 
    // Route to get completed tasks where the user is the leader
    router.get('/issues/leader/completed', isLogin, issueController.getCompletedLeaderTasks);
    // get issues which ar in progress
    router.get('/issues/in-progress', isLogin, isAdmin, issueController.getInProgressIssues);
    // router.get('/issues/completed', isLogin, isAdmin, issueController.getCompletedIssues);
    // Route to get all volunteer issues
    router.get('/volunteer-issues', isLogin, isVolunteer, issueController.getVolunteerIssues);  

    // Route for assigning a sub-task
    router.post('/issues/:issueId/assign-sub-task', isLogin,issueController.assignSubTask);

    router.get('/issues/:issueId/sub-tasks', isLogin, issueController.getAssignedSubTasks);
    // Route for volunteers to report on a sub-task
    router.post('/issues/:issueId/sub-tasks/:subTaskId/report', isLogin, issueController.reportOnSubTask);
    ...
    router.get('/in-progress-issues-reports', isLogin, isAdmin, issueController.getInProgressIssuesWithUpdates);
    ...
    router.get('/completed-issues', isLogin, isAdmin, issueController.getCompletedIssues);
