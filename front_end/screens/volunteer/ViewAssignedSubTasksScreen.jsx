...
const ViewAssignedSubTasksScreen = ({ route }) => {
  ...
  const [statusValues, setStatusValues] = useState({}); // Store selected statuses per sub-task

  const fetchSubTasks = async () => {
    const token = await AsyncStorage.getItem('token');
    try {
      const response = await axios.get(
        `${BASE_URL}/api/issueReporting/issues/${issueId}/sub-tasks`,
        {
          headers: { Authorization: `Bearer ${token}` },
        }
      );
  ...
  <DropDownPicker
    ...
    setValue={(callback) =>
      setStatusValues((prev) => ({
        ...prev,
        [item._id]: callback(prev[item._id]),
      }))
    }
    ...
    containerStyle={styles.dropdownContainer}
    style={styles.dropdown}
    dropDownContainerStyle={styles.dropDownBox} // Added dropdown container style
    textStyle={styles.dropdownText} // Style for dropdown text
    placeholderStyle={styles.dropdownPlaceholder} // Style for placeholder
  />
) : (
  <TouchableOpacity
    style={styles.openDropdownButton}
    onPress={() => setOpenDropdown(item._id)}
  >
    <Text style={styles.openDropdownButtonText}>Change Status</Text>
  </TouchableOpacity>
)}
              ...
              <ScrollView>
                ...
                {reportData && reportData.length > 0 ? (
                  reportData.map((report, index) => (
                    <View key={index} style={styles.reportCard}>
                      <Text style={styles.reportText}>
                        Reported By: {report.updatedBy.name}
                      </Text>
                      <Text style={styles.reportText}>Description: {report.description}</Text>
                      <Text style={styles.reportText}>
                  ...
                  <Text style={styles.noReportText}>
                    No reports available for this sub-task.
                  </Text>
                )}
                <TouchableOpacity
                  style={styles.closeButton}
